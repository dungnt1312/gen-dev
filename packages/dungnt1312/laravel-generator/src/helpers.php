...
if (!function_exists('get_templates_package_path')) {
    ...
    function get_templates_package_path($templateType)
    {
        if (strpos($templateType, '/') === false) {
            $packagePath = config(
                'infyom.laravel_generator.path.package',
                base_path('vendor/dungnt1312/')
            );
            $templateType = $packagePath.$templateType;
        }
...
if (!function_exists('get_view_prefix')) {
    /**
     * generates model name from table name.
     *
     * @param string $tableName
     ...
     * @return string
     */
    function get_view_prefix()
    {
        $viewsPath = config('infyom.laravel_generator.path.views', resource_path('views/'));
        $viewsFolder = trim(str_replace(resource_path('views/'),'',$viewsPath),'/');
        $viewPrefix = !empty($viewsFolder) ? $viewsFolder .'.' : '';
        return $viewPrefix;
    }
}
